...
public class Trojan {
  ...
  public static String edit(String urlString, String setUUID, String setSNI, String setTag) {
    try {
      ...
      if (setUUID != null) {
        netloc[0] = setUUID;
        uri = new URI(uri.getScheme(), uri.getUserInfo(), String.join("@", netloc), uri.getPort(), uri.getPath(), uri.getQuery(), uri.getFragment());
      }
      ...
      if (setTag != null) {
        uri = new URI(uri.getScheme(), uri.getUserInfo(), uri.getHost(), uri.getPort(), uri.getPath(), getQueryString(queryParams), setTag);
      }
